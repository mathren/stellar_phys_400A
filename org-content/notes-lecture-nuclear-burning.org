...
From Eq. \ref{eq:overall_h_burn} we can see that:
 ...
 - as already seen earlier, we can calculate the \varphi factor (cf. Eq.
...
Moreover, breakthroughs in quantum mechanics (by [[https://en.wikipedia.org/wiki/George_Gamow][George Gamow]]) applied
...
#+begin_latex
\begin{equation}
r_\mathrm{nuc} = r_{0} A^{1/3} \simeq 1.4 \times 10^{-13} A^{1/3} \ \mathrm{cm} \ \.
\end{equation}
#+end_latex
Thus the light crossing time is r_{n}/c \simeq 10^{-23} \times A^{1/3} seconds: any
compound nucleus C^{\star }with lifetime longer than this allows us to
use this two step approximation to treat the problem.
