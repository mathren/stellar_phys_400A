...
generation per unit time $\varepsilon_\mathrm{nuc}$ due to nuclear burning (which will
...
L>L_\mathrm{nuc}), then it will lose too much energy, meaning it will contract,
...
see in a moment), until L_\mathrm{nuc} reaches L.
Viceversa, if L_\mathrm{nuc} > L for some reason (e.g., there is a thermonuclear
