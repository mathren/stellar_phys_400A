...
- Using =MESA-web=, which contains tabulated data for nuclear reactions
  ...
  burning of a 20M_{}_{\odot} star and
  use the =trimmed_history.data= output to:
  ...
  *N.B.:* Computing the evolution of massive stars through /all/ the
  ...
  telling you if the model was successful or not. If your model is
  not, mention which input you used (listed in that file) and list the
  composition and typical duration up to where your model evolved (or
  try fiddling with the settings!).

  I have tried the following settings to get at least to an oxygen
  composition in the core (so not till the very end, feel free to
  explore different combinations!):
  - Initial Mass: 15M_{\odot}
  - Burning Modifiers: =none=
  - Nuclear Reaction Network: =approx21= (this uses 21 isotopes)
  - Sampled Nuclear Reaction: =none=
  - Initial Metallicity: 0.02
  - Mixing Length Alpha: 2.0
  - Mixing Length Theory Implementation: =Cox= (this is a specific
    implementation of MLT)
  - Convective Overshoot f: 0.355
  - Convective Overshoot f0: 0.02 (these "overshooting" parameters
    determine convective boundary mixing )
  - Semi-Convection Alpha: 0.0
  - Thermohaline Alpha: 0.0
  - Thermohaline Mixing Implementation: Kippenhahn
  - boundary Mixing: =none=
  - Red Giant Branch Wind Scheme: =Dutch= (specifies the wind mass loss
    rate from the star)
  ...
  - Variance Control Target: =1e-2= (specifies the time resolution)
  - Mesh Delta Coefficient: =1.0= (specifies the mass resolution)
  - dX_nuc_drop_min_X_limit: 1e-1 (specifies the resolution in nuclear
    composition)
  - MESA Release: r12778
