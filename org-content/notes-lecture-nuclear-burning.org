...
#+begin_latex
\begin{equation}
V \equiv V(r) = V_\mathrm{Coulomb} + V_\mathrm{n} + \frac{\ell(\ell+1)\hbar^{2}}{2m_{aX}r^{2}} \ \ ,
\end{equation}
#+end_latex
...
#+begin_latex
\begin{equation}
V_\mathrm{n}(r) = a_\mathrm{vol} A - a_\mathrm{surf} A^{2/3} - a_\mathrm{Coulomb} \frac{Z^{2}}{A^{1/3}} - a_\mathrm{sym} \frac{(N-Z)^{2}}{A} + a_\mathrm{coupling} \ \ ,
\end{equation}
#+end_latex
...
r_{n} = r_{0} = 1.3 \times 10^{-13} cm. The temperature threshold we have derived
