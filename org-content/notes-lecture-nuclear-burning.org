...
#+begin_latex
\begin{equation}
r_\mathrm{nuc} = r_{0} A^{1/3} \simeq 1.4 \times 10^{-13} A^{1/3} \ \mathrm{cm} \Rightarrow \tau_{cross} = \frac{r_\mathrm{nuc}}{c} \simeq 10^{-23} A^{1/3} \ \mathrm{s} \ \ .
\end{equation}
#+end_latex
...
(nuclei/ions+electrons), and \Chi = \sum_{i}_{}^{} Z_{i}^{2} (N_{i}/N) + N_{e}/N with N_{i} and N_{e}
