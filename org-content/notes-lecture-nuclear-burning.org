...
- just outside r_{n}\equiv r_{nuc} there is the maximum height of the Coulomb barrier
  E_{C} = Z_{a} Z_{X} e^{2}/r_{nuc}.
...
\leq r_{nuc}. Assuming the energy is just coming from the thermal energy of
the gas:

#+begin_latex
\begin{equation}
E \simeq k_{B} T \geq E_{C} = \frac{Z_{a}Z_{X}e^{2}}{r_{nuc}} \geq \frac{e^{2}}{r_{0}} \Rightarrow T\geq \frac{e^{2}}{r_{0} k_{B}} \simeq 10^{10} \ \mathrm{K} \ \ ,
\end{equation}
#+end_latex
where we assume Z_{a} = Z_{x} = A = 1 to minimize the Coulomb barier, so
r_{nuc} = r_{0} = 1.3 \times 10^{-13} cm. The temperature threshold we have derived
