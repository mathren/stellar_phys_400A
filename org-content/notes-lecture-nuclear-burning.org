...
case only the /total/ leptonic number is conserved).
...
at infinity higher than the maximum of the Coulomb barrier, so that
$r_{c} \leq r_{n}$. Assuming the energy is just coming from the thermal
energy of the gas:
