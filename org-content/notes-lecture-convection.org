...
#+begin_latex
\begin{equation}
\frac{d\rho}{dr}_{i} = -\frac{P}{H_{P}} \frac{d\rho}{dP} \ \ .
\end{equation}
#+end_latex
...
radiative gradient. *N.B.:* We have worked out above that in the
unstable situation \nabla_{rad} is steeper than \nabla_{ad}, so the form above also
guarantees that \Delta T is actually positive, as we expect for a bubble
raising and carrying an excess thermal energy compared to the
background.
