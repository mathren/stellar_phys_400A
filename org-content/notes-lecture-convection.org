...
#+begin_latex
\begin{equation}\label{eq:v_conv_MLT}
 v_\mathrm{conv} = \sqrt{\frac{1}{2}\ell }
\end{equation}
#+end_latex


*** Estimate of the convective velocity
...
F_\mathrm{conv} = \rho c_{P} T \alpha_\mathrm{MLT}^{2} \sqrt{\frac{1}{2} \frac{Gm(r)}{r^{2}} H_{P}} (\nabla_\mathrm{rad}-\nabla_\mathrm{ad})^{3/2} \ \ .
\end{equation}
#+end_latex
...
assume a monoatomic gas for c_{P} and use the ideal gas EOS to state and
hydrostatic equilibrium equation to state:
...
Using L(r)/4\pi r^{2} \sim L/R^{2} and putting everything together we obtain:
...
- We assume that the average distance travelled by a bubble is
  ...
  to approximate in spherical symmetry.

* Homework
- Using the definition of pressure scale height and the hydrostatic
  equilibrium equation, show that P/\rho = 1/(g H_{p})
