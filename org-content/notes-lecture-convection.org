...
(small) amount \delta \ell and discuss how the parcel reacts: if things act to
move the parcel of gas back towards its original position we have a
stable situation, if instead a seed initial displacement \delta \ell result in
...
equalized through sound-waves. Therefore, throughout the path \delta \ell and at
the final position we have P_{bubble} = P_{environment}.

Let's calculate the density. Since we assume \delta \ell to be "small" (w.r.t.
the relevant spatial scales in the star), a first order approximation
is sufficient: \rho_{bubble} = \rho_{1} + (d\rho/dr)_{ad} \delta \ell. The relevant way to calculate
...
We want to compare \rho_{bubble} after the displacement by \delta \ell to the
...
transport and the EOS). Therefore \rho_{2} = \rho_{1} + (d\rho/dr)_{env }\delta \ell \equiv
\rho_{1} + (d\rho/dr)_{rad }\delta \ell, because we are assuming the surrounding
