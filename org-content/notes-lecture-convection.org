...
matter with net zero flux that is carrying energy $\tau_\mathrm{conv} = \Delta
r/v_\mathrm{conv}$. This is the "convective turnover timescale", where
$\Delta r$ is the spatial extent of a convective region, and
$v_\mathrm{conv}$ is the velocity of the blobs of matter moving
around.
...
\mathrm{Instability\ if:} \ \rho_\mathrm\mathrm{bubble} \leq \rho_{2}  \Rightarrow
\left(\frac{d\rho}{dr}\right)_\mathrm{ad} \leq
\left(\frac{d\rho}{dr}\right)_\mathrm{env} \equiv \left(\frac{d\rho}{dr}\right)_\mathrm\mathrm{rad} \ \ .
\end{equation}
#+end_latex
...
\mathrm{Instability\ if:} \ \frac{\partial \log(T)}{\partial \log(P)}_\mathrm\mathrm{rad} = \nabla_\mathrm\mathrm{rad} > \frac{\partial \log(T)}{\partial \log(P)}_\mathrm{ad} = \nabla_\mathrm{ad} \ \ ,
\end{equation}
#+end_latex
...
\nabla_\mathrm\mathrm{rad} = \frac{3\kappa L P}{64\pi G m \sigma T^{4}} \propto \kappa L\ \ ,
\end{equation}
#+end_latex
...
#+begin_latex
\begin{equation}
\Delta T = T_\mathrm\mathrm{bubble} - T_\mathrm{env} = \left(T_{1} + \frac{dT}{dr}\rvert_\mathrm\mathrm{bubble}\ell \right) - \left(T_{1} + \frac{dT}{dr}\rvert_\mathrm{env}\ell \right) = \left(\frac{dT}{dr}\rvert_\mathrm\mathrm{bubble} - \frac{dT}{dr}\rvert_\mathrm{env} \right)\ell \ \ .
\end{equation}
#+end_latex
...
#+begin_latex
\begin{equation}
\Delta\rho \simeq \left(\rho_{1} +\frac{d\rho}{dr}\rvert_\mathrm{ad} \ell\right) - \left(\rho_{1} +\frac{d\rho}{dr}\rvert_\mathrm\mathrm{rad} \ell\right) \ \ ,
\end{equation}
#+end_latex
...
(\nabla_\mathrm{rad} - \nabla_\mathrm{ad})^{1/2} c_\mathrm{sound} \simeq
(\nabla_\mathrm{rad} - \nabla_\mathrm{ad})^{1/2} v_\mathrm{Thermal}$. We have just
seen that for efficient convection, the superadiabaticity is small,
...
Nevertheless, even a velocity of $v_\mathrm{conv} \simeq 10^{-4} c_{sound}$ is
