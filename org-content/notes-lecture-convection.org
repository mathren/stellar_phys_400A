...
#+begin_latex
\begin{equation}
\Delta T = T_\mathrm{Bubble} - T_\mathrm{env} = \left(T_{1} + \frac{dT}{dr}_\mathrm{Bubble}\ell \right) - \left(T_{1} + \frac{dT}{dr}_\mathrm{env}\ell \right) = \ell \left(\frac{dT}{dr}_\mathrm{Bubble} - \frac{dT}{dr}_\mathrm{env} \right) \ \ .
\end{equation}
#+end_latex
