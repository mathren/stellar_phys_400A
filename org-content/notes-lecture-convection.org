...
matter with net zero flux that is carrying energy \tau_\mathrm{conv} = \Delta r/v_\mathrm{conv}.
This is the "convective turnover timescale", where \Delta r is the spatial
extent of a convective region, and v_\mathrm{conv} is the velocity of the blobs
...
To estimate the convective velocity v_\mathrm{conv} we can consider the work
...
Conveniently in Eq. \ref{eq:v_conv_MLT} both v_\mathrm{conv} and \ell are squared: the same
...
in the convective energy flux, v_\mathrm{conv} and \ell, which allows us to
...
To obtain an order of magnitude estimate, we can substitute in F_\mathrm{conv}
...
Eq. \ref{eq:v_conv_MLT_estimate} derived above tells us that v_\mathrm{conv}
...
chemicals by convection D_\mathrm{conv} = 1/3 \times v_\mathrm{conv} \ell = 1/3 \times v_\mathrm{conv}
...
takes much longer than the convective turnover timescale \tau_\mathrm{conv}.
...
model for d v_\mathrm{conv} / dt, see e.g., [[https://ui.adsabs.harvard.edu/abs/1969ApJ...157..339A/abstract][Arnett 1969]], [[https://ui.adsabs.harvard.edu/abs/1974ApJ...190..609W/abstract][Wood 1974]], [[https://ui.adsabs.harvard.edu/abs/1977ApJ...214..196G/abstract][Gough 1977]],
