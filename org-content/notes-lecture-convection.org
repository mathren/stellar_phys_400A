...
#+begin_latex
\begin{equation}
\Delta\rho \simeq \left(\rho_{1} +\frac{d\rho}{dr}_\mathrm{ad} \ell\right) - \left(\rho_{1} +\frac{d\rho}{dr}_\mathrm{rad} \ell\right) \simeq  \ \ ,
\end{equation}
#+end_latex
...
E_\mathrm{kin, Bubble} = B\cdot\ell \Rightarrow \frac{1}{2}\rho_\mathrm{Bubble} v_\mathrm{conv}^{2}  \ \ .
\end{equation}
#+end_latex
...
- Using the definition of pressure scale height and the hydrostatic
  equilibrium equation, show that P/\rho = 1/(g H_{p}) \Rightarrow c_{sound}^{2}
  \simeq gH_{p}
