...
#+begin_latex
\begin{equation}
\frac{n_{e} n_{+}}{n_{0}} = 2\frac{(2\pi m_{e} k_{B}T)^{3/2}}{h^{3}} \exp\left(-\frac{\chi}{k_{B}_{}T}\right) \ \ ,
\end{equation}
#+end_latex
...
we can introduce the ionization fraction x = n_{+}/(n_{0} + n_{+}), and
rewrite the ideal gas pressure as the sum of the contributions of
electron, ion, and neutral atom P = P_{e} + P_{+} + P_{0}. Using the
approximation that electrons don't contribute to the mass, that is \rho =
(n_{+} + n_{0})m_{u}, and the definition of the ionization fraction, this
gives P = (1+x)\rho T
