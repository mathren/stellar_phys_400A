...
*** Fully degenerate electron gas
  ...
  #+begin_latex
  \begin{equation}
  g_{e}(p)dp = q_{s}_{} \frac{4\pi p^{2}_{}^{}}{h^{3}}dp \equiv \frac{8\pi p^{2}_{}^{}}{h^{3}}dp \ \mathrm{for} p\leq p_{f} \ \mathrm{otherwise} 0 \ \ .
  \end{equation}
  #+end_latex
  ...
  #+begin_latex
  \begin{equation}
  n_{e} = \int_{0}^{+\infty} g_{e}(p)dp = \frac{8\pi}{3h^{3}}p_{f}^{3} \Rightarrow p_{F} = h\left(\frac{3}{8pi}n_{e}\right)^{1/3} \ \ .
  \end{equation}
  #+end_latex
...
**** Relativistic electron gas
  ...
  move inwards in a star where these effects matter.
...
- Estimate T for a WD assuming its supported by non-relativistic
