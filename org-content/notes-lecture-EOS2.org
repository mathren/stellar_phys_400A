...
- Clayton's problem 2-59: Consider a star entirely supported by
  ...
  M_{\odot} for \mu_{e} \simeq 1/2 (note the \mu_{e}^{2} dependence!). What does
  this specific value of the mass (for a given composition, i.e., \mu_{e})
  mean for stars supported by ultra-relativisti electron degeneracy
  pressure? What equation of stellar structure (of the ones we have
  seen so far) /cannot/ be satisfied for larger values of the mass?
