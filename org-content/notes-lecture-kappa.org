...
the star compensates for the surface energy losses (its luminosity L).
...
to a black body distribution in the stellar interior:
...
is usually referred to as the Rosseland mean opacity.
...
Consider the dynamics of an /optically thick/ gas. If sufficiently hot,
we need to include in the momentum equation the radiation pressure,
and if it is even hotter, we know that the radiation pressure will be
the dominant pressure term. Let's assume to start from a situation
where our optically thick gas is self-gravitating and in hydrostatic
equilibrium. Using the momentum balance and the equation for radiative
energy transport, derive the maximum luminosity for which hydrostatic
equilibrium can be sustained: this is the so-called Eddington
luminosity L_{Edd}, which has multiple applications from stellar to
accretion physics!

For L \geq L_{Edd} the radiation pressure wins over gravity, and hydrostatic
equilibrium cannot occur. Consider the opacity dependence of L_{Edd}. In
his original derivation Eddington only considered electron scattering.
However, as you have just demonstrated, the generic gas opacity with
all its contributions appears if one derives it from the equations for
the stellar interior.

If \kappa(m) decreases as we move outward to larger mass coordinates m,
L_{edd} \propto \kappa^{-1} will decrease, sometimes /faster/ than L(m),
leading to the situation L(m) \geq L_{Edd}(\kappa(m)) which is problematic.

Using =MESA-web=, compute a X M_{\odot} star saving multiple profiles, and
plot \kappa(m) and or \kappa(T) for the outer layers. Plot also L(m) and L_{edd}(m,
\kappa) and identify what happens in layers where L exceeds L_{edd}. We will
