...
bottleneck of the weak reion in the pp chain is to use metal ions as
...
#+CAPTION: \varepsilon_\mathrm{nuc}/\rho as a function of central temperature T_{c} (bottom axis) or initial mass (top axis) for the pp chain (blue) and CNO cycle (orange) based on a grid of MESA models at the onset of hydrogen core burning. Dashed lines are linear fits. Modified from a figure by R. Townsend.
#+ATTR_HTML: :width 75%
