...
opens up, the (in)famous ^{12}C(\alpha,\gamma)^{16}O reaction.
...
^{12}C(\alpha,\gamma)^{16}O reaction, the composition of the core is now a mixture
...
^{12}C+^{12}C \rightarrow^{20}Ne+\alpha (Q\sim 4.616MeV)
...
to /photodisintegrate/ the heavier (but not magic) nucleus of ^{20}Ne
before oxygen burns.
