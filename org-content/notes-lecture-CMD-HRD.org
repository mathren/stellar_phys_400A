...
much about them now, the important point is that we can put together
on the same (or tightly related) axes information for an observed
population of stars (e.g., the /Gaia/ CMD above) and for a theoretical
model of individual stars (e.g., the Hertzsprung-Russel evolutionary
diagram here). This mixes two ontologically different elements (models
and observations), but provides a powerful tool to interpret the
observations.
