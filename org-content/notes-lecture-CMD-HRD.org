...
#+being latex label:Planck
F_{\lambda} d\lambda = \frac{2\pi hc^{2}}{\lambda^{5}}\frac{d\lambda}{e^{hc/\lambda k_{B} T} -1} \ \ .
#+end latex

Note that this only depends on the temperature of the interior T. This
produces the "continuum" of the spectrum, and can be thought of the
spectrum of light coming from the stellar interior:
...
#+CAPTION: Schematic representation of the Sun spectral energy distribution on the ground.
#+ATTR_HTML: :width 50%
#+ATTR_HTML: :alt


Typically, data reduction pipelines are used to remove the "slope" due
to the underlying continuum and focus only on the absorption lines. An
example of the final product for a Sun-like star is
