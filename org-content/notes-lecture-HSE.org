...
:Questions:
 - *Q*: can there be other variables that one can use as independent
variable for the description of the stellar structure?
:end:
...
Let's now consider the equation of motion of a parcel of stellar gas,
F = dp/dt = ma (for constant m), or often more conveniently in fluid
dynamics, work per unit volume with f = F/V and thus f=\rho a with \rho =
m/V and V volume.
...
We can introduce the gravitational acceleration \mathbf{g} = - \nabla\Phi, which
in spherical symmetry only has a non-zero radial component \Rightarrow g = - d\Phi/dr
...
shell of mass dm = 4\pi r^{2}\rho dr is thus just -gdm = -Gm\rho/r^{-2}, where
the minus sign is to explicitly indicate that this force points
towards the center of the star, and

*** Pressure gradient
The other contribution we need to include in our f = \rho a equation is from
the pressure. We could already use dimensional analysis to guess in
what form pressure can enter the l.h.s. of the equation:
...
Let's have a slightly more formal look at how this work.
