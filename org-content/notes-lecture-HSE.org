...
\ref{eq:HSE}, we should take the entire mass m\rightarrow M=\int dm and outer radius
R, which gives:

#+begin_latex
\begin{equation}
P_\mathrm{center} = \frac{GM^{2}^{}}{4\pi R^{4}} \ \ ,
\end{equation}
#+end_latex

Plugging in the numbers for the Sun this gives P_{center}\simeq 9\times
10^{14} dyne/cm^{2}\simeq 10^{9}^{} atmospheres. Although this a is very imprecise
...
conservation f = \rho a \equiv \rho \partial^{2}r/\partial_{t}^{2} we have

#+begin_latex
\begin{equation}
\rho \frac{\partial^{2} r}{\partial r^{2}} = -\frac{dP}{dr} -\frac{Gm}{r^{2}}\rho \ \ ,
\end{equation}
#+end_latex
