...
frequency/ \kappa_{\nu}\rightarrow\kappa. With these two hypothesis (plane
parallel and gray atmosphere) we can rewrite \tau_{\nu}\rightarrow\tau and
...
With the second hypothesis of a gray atmosphere, we can now integrate
this in d\nu from 0 to +\infty and obtain:

#+begin_latex
\begin{equation}
\cos(\theta) \frac{dI}{dz} = -\kappa\rho(S-I) \ \ ,
\end{equation}
#+end_latex
