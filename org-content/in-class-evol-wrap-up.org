...
Above is a =pgstar= movie of the He flash(es) in a 1M_{\odot} star. Note
the panels are /different/ than in the =MESA-web= configuration, and the
HRD does /not/ show the pre-main sequence.
...
The envelope that has not been lost during the RG phase is typically a
bit less luminous and extended. Very approximately, increasing the
mass, L stays roughly constant, and T_{eff} increases forming the
so-called "horizontal branch".
...
*** Single-star evolution path to Wolf-Rayet
