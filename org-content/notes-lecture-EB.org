...
\ref{eq:semimajor_axes} one can still determine the mass ratio q=M_{2}/M_{1}!
...
We can take Eq. \ref{eq:Kepler3} and multiply
it by sin^{3}(i) to make the projection of the semi-major axis of the
orbit on the line of sight, a \times sin(i), appear:
