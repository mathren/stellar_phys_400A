...
#+begin_latex
\begin{equation}
J_\mathrm{tot} = J_{orb}_{}+J_{1}+J_{2} \ \ ,
\end{equation}
#+end_latex
...
the thermal timescale of the donor star (e.g., for case B RLOF),
making it impossible for tides to circularize the system before/during
mass transfer.
...
The last term is usually the dominant one.
