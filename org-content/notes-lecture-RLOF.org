...
#+begin_latex
\begin{equation}
\Phi(\mathbf{r}) = -\frac{GM_{1}}{\lvert \mathbf{r}-\mathbf{r}_{1}\rvert}-\frac{GM_{2}}{\lvert \mathbf{r}-\mathbf{r}_{2}\rvert}-\frac{1}{2}(\mathbf{\Omega}\times\mathbr{r})^{2} \ \ ,
\end{equation}
#+end_latex
...
zeros, meaning points where there is no net force. These are called
Lagrangian points:
...
#+begin_latex
\begin{equation}
R_{RL,i}_{} = a \frac{0.49 q_{i}^{2/3}}{0.6q_{i}^{2/3}+\ln(1+q_{i}^{2/3})} \ \ ,
\end{equation}
#+end_latex
...
*** Dynamically stable mass-transferSS
**** Case A
