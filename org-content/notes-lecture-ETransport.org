...
derive such equation.

Above, we have defined: L = 4\pi r^{2} F (where now both L and F include
the contribution from conduction and radiation).


The energy density per unit mass and volume in a star is often
indicated with \varepsilon, such that [\varepsilon] = [E]/[m][L^{3}].
