...
\frac{1}{\kappa} = \frac{1}{\kappa_\mathrm{rad}} + \frac{1}{\kappa_{cond}} \ \ .
\end{equation}
#+end_latex
...
:Question:
- *Q*: if we compress the gas (\delta\rho > 0 because \rho increases), without
  adding/extracting heat (\delta q = 0) what happens to the internal
  energy?
:end:
...
- energy loss by some particle escaping, this can be for example
  ...
  decrease the energy by extracting heat, since as soon as they are
  produced neutrinos will leave the star with no further interaction
  (with the exception of neutron stars). The neutrino energy cooling
  rate per unit mass is indicated by \varepsilon_{\nu} and it has always a *negative*
...
We still need an equation for the convective energy transport.
