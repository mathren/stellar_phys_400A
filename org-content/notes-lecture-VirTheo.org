...
the units of a specific energy per unit mass, and thus P/\rho \prop u with u
specific internal energy. For now, let's call the proportionality
constant (\Gamma-1). It can be shown using the first law of thermodynamics
assuming an adiabatic process (i.e., no heat exchange) that \Gamma\equiv\Gamma_{1} is
the first adiabatic index, so P/\rho = (\Gamma_{1}-1)u, so we can write:
