...
#+begin_latex
\begin{equation}
 2\mathrm{KE}+\mathrm{E_\mathrm{pot} = \frac{1}{2}\ddot{I}
\end{equation}
#+end_latex
...
\end{equation}
#+end_latex
Note that if we had kept a non-zero \rho a term in Eq. \ref{eq:HSE}, we
...
the l.h.s. By dimensional analysis we know it has to have the
