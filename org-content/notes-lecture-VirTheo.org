...
\int_{0}^{M} -\frac{Gm}{4\pi r^{4}} \times 4\pi r^{3} dm  = \int_{0}^{M} -\frac{Gm}{r}dm \equiv \mathrm{U} \ \ .
\end{equation}
#+end_latex
...
\end{equation}
#+end_latex
We have already interpreted the r.h.s. as the gravitational potential
energy (normalized to zero at infinity) in Eq. \ref{eq:pot}. Let's
interpret the l.h.s. - for this we need a relation between the
local pressure P and the local density \rho, which is an equation of
state.
